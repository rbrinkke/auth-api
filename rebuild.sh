#!/bin/bash
# Rebuild script for auth-api
# Steps:
# 1. Stop all containers
# 2. Remove auth-api images (NOT volumes!)
# 3. Rebuild and start
...
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}Auth-API Rebuild Script${NC}"
echo -e "${BLUE}========================================${NC}"
echo ""
...
# Step 2: Remove auth-api images (NOT volumes!)
echo -e "${YELLOW}Step 2: Removing auth-api images...${NC}"
# Get image names for this project
IMAGE_NAMES=$(docker images --format "{{.Repository}}:{{.Tag}}" | grep "auth-api" || true)
...
if [ -n "$IMAGE_NAMES" ]; then
    ...
    echo "No auth-api images found to remove"
...
# Show auth-api logs (last 10 lines)
echo -e "${YELLOW}Auth-API Logs (last 10 lines):${NC}"
echo ""
docker compose logs --tail 10 auth-api
echo ""
...
echo "  docker compose logs -f auth-api    # Follow logs"
